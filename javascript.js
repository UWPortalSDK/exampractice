angular.module('portalApp')
    .controller('exampracticeCtrl', ['$scope', function($scope) { //There is a
        ...
        $scope.che102questions = [{
                title: '★ Questions',
                ONE: 'What is the molar mass of CO2?'

            },
            {
                title: '★★ Questions',
                ONE: 'The  Province  of  Nova  Scotia  aspires  to  meet  1/10thof  its  domestic  household  power requirements  through  tidal  power.  Census  results  from  2011  indicate  that  there  are 390,000  households  in  the  Province  and  that  on  average  750  households  consume 2.5MW  of  power.  A  European  manufacturer  has  estimated  the  capital  cost fortidal turbines to be €10 million per 3350hp of power output.'
            },
            {
                title: '★★★ Questions',
                ONE: 'If the percent yield of chromium (Cr) is 82%, how many grams of silicon (Si) are required to make 105 g of chromium according to the following unbalanced reaction?'
            }, 
            {
                title: '★★★★ Questions',
                ONE: 'A 100.0g sample of a given hydrocarbon when combused produces 338.0g of CO2 and 69.2g of H2). Find the empirical formula of the hydrocarbon'
            }, 
            {
                title: '★★★★★ Questions',
                ONE: 'A  van  der  Waals  gas  is  known  to  have  strong  attractive  intermolecular  forces. Compared  to  an  ideal  gas  at  the  same  volume  andtemperature, the pressure is expected to be lower in the real gas.'
            },

        ];

        // Show main view in the first column as soon as controller loads
        $scope.portalHelpers.showView('exampracticeMain.html', 1); //1 means page 1  //exampracticeMain.html means main is on page 1
    ...
    {       
      ...
      /*
        ...
        
        {
            // Make the item that user clicked available to the template
            $scope.detailsItem = item;
            $scope.portalHelpers.showView('exampracticeQuestion.html', 6);
        };
                $scope.showOneQues = function(item) 
        {
            $scope.detailsItem = item;
            $scope.portalHelpers.showView('exampracticeExam.html', 3);
        };
